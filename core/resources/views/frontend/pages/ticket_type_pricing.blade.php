        ...
        <div class="container pt-5">
            ...
            <div class="row pt-3 g-5">
                @if ($errors->any())
                    ...
                    @endif
                @foreach ($allTicketType as $item)
                    <div class="col-lg-4 col-md-12 mb-4 text-white">
                        <div class="card card1 h-100">
                            <div class="card-body">
                                <h5 class="card-title text-capitalize">{{$item->name}} <span class=" text-danger">@if ($item->name == 'basic') /Free
                                    
                                @endif</span></h5>
                                <span class="h2 pt-4">{{$item->priceCurrency->symbol}}{{$item->price}}</span>

                                <a href="{{route('ticketType.Pricing.place_order',$item->id)}}" class="text-decoration-none w-100">
