        ...
        <div class="container py-5 mx-auto">
            <div class="row g-4">
                <div class="col-lg-8 col-md-12 col-12">
                    ...
                    <div class="my-4">
                        <h3 class="text-white fs-5">{{ $smileTvDetails->title }}
                        </h3>
                        <p class="text-white">146,114 view <span> @php
                            $date = $smileTvDetails->date;
                            echo date('M d, Y ', strtotime($date));
                        @endphp</span></p>
                        <form action="" method="post" id="addSmileTvLikeForm">
                            <div class="d-flex pt-3">
                                <div class="pe-5 d-flex d-none" style="cursor: pointer;">
                                    <input type="text" name="smile_tv_id" value={{ $smileTvDetails->id }}>
                                </div>
                ...
                <div class="col-lg-4 col-md-12 col-12">
                    ...
                    <form action="" method="post" id="addSmileTvCommentForm">
                        @csrf
                        <div class="row">
                            <div class="form-floating mt-4 col-lg-10 col-md-10 d-none">
                                <input type="text" name="smile_tv_id" id="SmileTvLikeId" value={{ $smileTvDetails->id }}>
    ...
    @push('js')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const source = "{{ $smileTvDetails->tv_link }}";
        ...
        {{-- ajax smile tv like & comment details --}}
        <script>
            $(document).ready(function() {
                ...
                // ---------------------------smile tv details like ajax----------------------
                $(document).on("click", "i#like", function(event) {
                    event.preventDefault();
                    $.ajax({
                        url: "{{ route('smile.tv.like') }}",
                        method: "POST",
                        data: {
                            "like": true,
                            'smile_tv_id': $('#SmileTvLikeId').val(),
                        },
                        success: function(res) {
                            if (res.status) {
                                $('#totalDisLike').html(parseInt(res.data.totalDisLike));
                                 $('#totalLike').html(parseInt(res.data.totalLike));
                ...
                // ---------------------------smile tv details Dislike ajax----------------------
                $(document).on("click", "i#disLike", function(event) {
                    event.preventDefault();
                    $.ajax({
                        url: "{{ route('smile.tv.dislike') }}",
                        method: "POST",
                        data: {
                            "like": true,
                            'smile_tv_id': $('#SmileTvLikeId').val(),
                ...
                // ---------------------------smile tv details comment ajax----------------------
                $(document).on("submit", "form#addSmileTvCommentForm", function(event) {
                    event.preventDefault();
                    $.ajax({
                        url: "{{ route('smile.tv.comment') }}",
                        method: "POST",
