        ...
        <div class="row justify-content-center mb-30-none">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            ...
                            <span>{{ $allManualEventPlanRequest->user->full_name ?? ' ' }}</span>
                        </div>
                        <hr>
                        
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            <span><strong>Email:</strong></span>
                            <span>{{ $allManualEventPlanRequest->user->email ?? ' ' }}</span>
                        </div>
                        <hr>
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            <span><strong>Phone:</strong></span>
                            <span>{{ $allManualEventPlanRequest->user->phone ?? ' ' }}</span>
                        </div>
                        <hr>
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            <span><strong>Country:</strong></span>
                            <span>{{ $allManualEventPlanRequest->user->country ?? ' ' }}</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-6 col-md-6 mb-30">
                <div class="card" >
                    ...
                    <div class="card-body">
                        @php
                            $manual_getway_fields = json_decode($allManualEventPlanRequest->detail);
                        @endphp
                        @if ($manual_getway_fields != null)
                            @foreach ($manual_getway_fields as $key => $item)
                                <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                                    <span><strong class="text-capitalize">{{ $key }}:</strong></span>
                                    @if (is_file(imagePath()['verify']['buy_event_plan']['path'] . '/' . $item->field_name))
                                        <span><img height="150px" width='150px' class=" img-fluid d-block mx-auto"
                                                src="{{ asset(imagePath()['verify']['buy_event_plan']['path'] . '/' . $item->field_name) }}"
                                                alt="Image"></span>
                                    @else
                        ...
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            ...
                            <span><strong>{{ $allManualEventPlanRequest->payment_getway ?? ' ' }}</strong></span>
                        </div>
                        <hr>
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            <span><strong>TRX:</strong></span>
                            <span><strong>{{ $allManualEventPlanRequest->transaction_id }}

                        </div>
                        <hr>
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            <span><strong> Paid Price:</strong></span>
                            <span><strong>{{ $allManualEventPlanRequest->paid_price ?? ' ' }}
                                    {{ $priceCurrency->symbol ?? ' ' }}</strong></span>
                        </div>
                        <hr>
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            <span><strong> Coupon Discount:</strong></span>
                            <span><strong>{{ $allManualEventPlanRequest->discount ?? ' ' }}
                                    {{ $priceCurrency->symbol ?? ' ' }}</strong></span>
                        </div>
                        <hr>
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            <span><strong>Charge:</strong></span>
                            <span><strong>{{ $allManualEventPlanRequest->charge ?? ' ' }}
                                    {{ $priceCurrency->symbol ?? ' ' }}</strong></span>
                        </div>
                        <hr>
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                            <span><strong>Total Amount:</strong></span>
                            <span><strong>{{ $allManualEventPlanRequest->final_amo ?? ' ' }}
                                    {{ $priceCurrency->symbol ?? ' ' }}</strong></span>
                        </div>
                        <hr>
                        ...
                        <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-end align-items-center">
                            @if ($allManualEventPlanRequest->status == 2)
                                ...
                                <a href="{{ route('admin.manual.getway.request.reject', $allManualEventPlanRequest->id) }}">Reject</a>
                            </button> --}}
                            @endif
                        </div>
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('admin.manual.event.request.reject', $allManualEventPlanRequest->id) }}"
                        method="POST">
                        @csrf
