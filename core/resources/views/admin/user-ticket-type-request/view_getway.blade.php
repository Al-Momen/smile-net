...
@section('content')
    ...
    <div class="table-content mt-5 ">
        <div class="row justify-content-center mb-30-none">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        ...
                        <span>{{ $allManualTicketTypeRequest->user->full_name ?? ' ' }}</span>
                    </div>
                    <hr>
                   
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        <span><strong>Email:</strong></span>
                        <span>{{ $allManualTicketTypeRequest->user->email ?? ' ' }}</span>
                    </div>
                    <hr>
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        <span><strong>Phone:</strong></span>
                        <span>{{ $allManualTicketTypeRequest->user->phone ?? ' ' }}</span>
                    </div>
                    <hr>
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        <span><strong>Country:</strong></span>
                        <span>{{ $allManualTicketTypeRequest->user->country ?? ' ' }}</span>
                    </div>

                    <hr>
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        <span><strong>Ticket Name:</strong></span>
                        <span class="text-capitalize text-danger"><u>{{ $allManualTicketTypeRequest->ticket_type->name ?? ' ' }}</u></span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="text-primary text-center">Subsctiption Plan User Data</h3>
                </div>
                <div class="card-body">
                    @php
                        $manual_getway_fields = json_decode($allManualTicketTypeRequest->detail ?? ' ');
                    @endphp
                    @if ($manual_getway_fields != null)
                        @foreach ($manual_getway_fields as $key => $item)
                            <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                                <span><strong class="text-capitalize">{{ $key }}:</strong></span>
                                @if (is_file(imagePath()['verify']['buy_ticket']['path']."/".$item->field_name))
                                <span><img height="150px" width= '150px' class=" img-fluid d-block mx-auto"
                                    src="{{ asset(imagePath()['verify']['buy_ticket']['path']."/".$item->field_name) }}" alt="Image"></span>
                                @else
                    ...
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        ...
                        <span><strong>{{ $allManualTicketTypeRequest->payment_getway ?? ' ' }}</strong></span>
                    </div>
                    <hr>
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        <span><strong>Ticket Price:</strong></span>
                        <span><strong>{{ $allManualTicketTypeRequest->ticket_type->price ?? ' ' }}
                                {{ $priceCurrency->symbol }}</strong></span>
                    </div>
                    <hr>
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        <span><strong> Coupon Discount:</strong></span>
                        <span><strong>{{ $allManualTicketTypeRequest->discount ?? ' ' }} {{ $priceCurrency->symbol ?? ' ' }}</strong></span>
                    </div>
                    <hr>
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        <span><strong>Charge:</strong></span>
                        <span><strong>{{ $allManualTicketTypeRequest->charge ?? ' ' }} {{ $priceCurrency->symbol ?? ' ' }}</strong></span>
                    </div>
                    <hr>
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-between align-items-center">
                        <span><strong>Total Amount:</strong></span>
                        <span><strong>{{ $allManualTicketTypeRequest->final_amo ?? ' ' }} {{ $priceCurrency->symbol ?? ' ' }}</strong></span>
                    </div>
                    <hr>
                    ...
                    <div class="block d-md-flex mb-sm-2 mb-md-0 justify-content-end align-items-center">
                        @if ($allManualTicketTypeRequest->status == 2)
                            ...
                            {{-- <button class="btn btn-danger">
                                <a href="{{ route('admin.manual.getway.request.reject', $allManualTicketTypeRequest->id) }}">Reject</a>
                            </button> --}}
                        @endif
                    </div>
